...
unsigned char pid[] = {"maovcaqtpynou1fw"};
...
void setup() {
  ...
  strip.setBrightness(100);  // 设置亮度最大255
...
void loop() {
  if (last_time > millis()){     //如果millis已重置，将所有时间记录清零
      last_time = 0;
  }
...
//      digitalWrite(LED_BUILTIN, led_state);
  ...
  //场景模式子程序 4-浪漫 5-缤纷 6-动感 7-梦幻 8-彩虹 9-流水
  if (dp_bool_value && dp_enum_value == 2) {
    switch(scene_mode){
       case 4:
         if (millis()- last_time >= 200) {
           ...
           for(int c=b; c<PIXEL_COUNT; c += 3) {
             strip.setPixelColor(c, strip.Color(141, 75, 187)); 
           }
       ...
       case 5:
         if (millis()- last_time >= 100) {
           ...
           strip.clear();
           for(int c=b; c<PIXEL_COUNT; c += 3) {
             strip.setPixelColor(c, strip.Color(random(0, 255), random(0, 255), random(0, 255))); 
           }
           strip.show(); // Update strip with new contents
           b++;
         }
       ...
       case 6:
         if (millis()- last_time >= 50) {
           ...
           strip.clear();
           for(int c=b; c<PIXEL_COUNT; c += 3) {
             strip.setPixelColor(c, strip.Color(127, 127, 127)); 
           }
           strip.show(); // Update strip with new contents
           b++;
         }
       ...
       case 7:
         if (millis()- last_time >= 150) {
           ...
           strip.clear();
           for(int c=b; c<PIXEL_COUNT; c += 3) {
             strip.setPixelColor(c, strip.Color(255, 102, 153)); 
           }
           strip.show(); // Update strip with new contents
           b++;
         }
...
unsigned char dp_process(unsigned char dpid,const unsigned char value[], unsigned short length)
{
  switch(dpid) {
    case DPID_SWITCH_LED:
      ...
      if (dp_bool_value) {
        //Turn on
        colorfill (strip.Color(  255, 0,  0)); //上一次状态
      } else {
        //Turn off
     ...
     switch(scene_mode){
       case 0:
        ...
        case 1:
          ...
          break;
        case 2:
          colorWipe(strip.Color( 255, 12,   0), 10);
          break;
        case 3:
          for(int i=0; i<PIXEL_COUNT; i++) { 
              int pixelHue = i * 65536L / PIXEL_COUNT;
              strip.setPixelColor(i, strip.gamma32(strip.ColorHSV(pixelHue)));
          }
          strip.show(); 
